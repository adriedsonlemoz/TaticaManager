...
const CupGroupTable = ({ group = [], color }) => {
  const C = THEME;
  ...
  return (
    <Box sx={{ bgcolor: C.card, border: `1px solid ${C.border}`, borderRadius: '10px', overflow: 'hidden', mb: 1 }}>
      <Box sx={{ px: 1.5, py: 0.8, bgcolor: `${color}15`, borderBottom: `1px solid ${color}40` }}>
        <Typography sx={{ color, fontWeight: 900, fontSize: '0.65rem', letterSpacing: 0.5 }}>TABELA DO GRUPO</Typography>
      </Box>
      <Box sx={{ px: 0.8, py: 0.5 }}>
        ...
        {group.map((team, index) => (
          <Box key={team.id || index} sx={{
            ...
            bgcolor: team.isPlayer ? 'rgba(34,197,94,0.08)' : index < 2 ? 'rgba(34,197,94,0.03)' : 'transparent',
            borderLeft: `3px solid ${team.isPlayer ? C.primary : index < 2 ? C.borderG : 'transparent'}`,
          }}>
