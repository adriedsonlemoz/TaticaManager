...
function zoneColor(index, serie, season, totalTeams) {
  if (serie === 'A') {
  ...
  if (index < 4) return D.green;
  if (serie === 'D') return 'transparent';
  const relegationCount = serie === 'C' && Number(season) === 2026 ? 2 : 4;
  if (index >= Math.max(0, totalTeams - relegationCount)) return D.red;
  return 'transparent';
}
...
export default function SeasonEndFinalTable({ vm }) {
  ...
  const serieDGroup = serie === 'D' ? vm.result.serieD?.group : null;
  return (
    <Box sx={{ bgcolor: D.card, border: `1px solid ${D.border}`, borderRadius: '10px', overflow: 'hidden' }}>
      <Box sx={{ px: 1.2, pt: 1, pb: 0.6 }}>
        <SeasonEndSectionTitle>CLASSIFICAÇÃO FINAL · SÉRIE {serie}{serieDGroup ? ` · GRUPO ${serieDGroup}` : ''}</SeasonEndSectionTitle>
      </Box>
      {rows.length === 0 ? (
        ...
        const zone = zoneColor(index, serie, vm.result.season, rows.length);
        return (
