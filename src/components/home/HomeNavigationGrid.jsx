      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 1 }}>
        {cards.map((card, index) => (
          <Box key={card.id} component="button" type="button" onClick={() => onNavigate(card.screen)} aria-label={`${card.label}: ${card.sub}`} sx={{
            position: 'relative', borderRadius: '16px', overflow: 'hidden', bgcolor: HOME_THEME.card,
            ...
            <Box aria-hidden="true" sx={{
              position: 'absolute', bottom: -6, right: -2, fontSize: '2.6rem', opacity: 0.06,
            ...
            <Box sx={{ px: 1.1, pt: 1, pb: 1, position: 'relative' }}>
              ...
              <Typography aria-hidden="true" sx={{ fontSize: '1.25rem', lineHeight: 1, mb: 0.4 }}>{card.emoji}</Typography>
