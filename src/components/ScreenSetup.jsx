...
const ScreenSetup = ({ setupData, setSetupData, handleStartNewGame, savesList, setScreen }) => {
  ...
  return (
    <Box sx={{ height: '100dvh', minHeight: 0, bgcolor: P.bg, background: `radial-gradient(ellipse at 60% 0%, rgba(16,185,129,0.07) 0%, transparent 50%), radial-gradient(ellipse at 10% 90%, rgba(16,185,129,0.04) 0%, transparent 40%), ${P.bg}`, display: 'flex', flexDirection: 'column', alignItems: 'center', overflow: 'hidden' }}>
      ...
      <Box sx={{ width: '100%', maxWidth: 560, px: { xs: 1.15, sm: 1.75 }, pt: 1.15, pb: 0.25 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.65 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Box><Typography sx={{ color: P.txt1, fontWeight: 900, fontSize: '1.02rem', fontFamily: '"Nunito",sans-serif', lineHeight: 1 }}>TÁTICA MANAGER</Typography><Typography sx={{ color: P.green, fontWeight: 900, fontSize: '0.62rem', letterSpacing: 3, lineHeight: 1 }}>NOVA CARREIRA · CLUBE REAL</Typography></Box>
          </Box>
          <Box sx={{ bgcolor: P.surface, border: `1px solid ${P.border}`, borderRadius: '8px', px: 1.1, py: 0.5, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}><Typography sx={{ color: P.green, fontWeight: 900, fontSize: '0.78rem' }}>{card}/{SETUP_TOTAL_STEPS}</Typography></Box>
        </Box>
        <SetupProgressBar step={card} />
      </Box>
      <Box sx={{ width: '100%', maxWidth: 560, px: { xs: 1.15, sm: 1.75 }, pb: 0.7, flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', opacity: entering ? 0 : 1, transform: entering ? 'translateY(10px)' : 'translateY(0px)', transition: 'opacity 0.17s ease, transform 0.17s ease' }}>
